...
class Student_profile extends Admin_Controller {
	...
	public function presences() {
		...
		$this->vars['title'] = 'Riwayat Presensi';
		$this->vars['student_presences'] = TRUE;
		$this->vars['content'] = 'students_presences';
		$this->load->view('backend/index', $this->vars);
	}
	 ...
	 * @return Object
	 */
	public function presences_pagination() {
		if ($this->input->is_ajax_request()) {
			$this->load->model('m_attendances');
			$student_id = __session('user_profile_id');
			$academic_year_id = __session('current_academic_year_id');
			$page_number = _toInteger($this->input->post('page_number', true));
			$limit = _toInteger($this->input->post('per_page', true));
			$offset = ($page_number * $limit);
			$query = $this->m_attendances->get_student_attendances($student_id, $academic_year_id, $limit, $offset);
			$total_rows = $this->m_attendances->get_student_attendances($student_id, $academic_year_id)->num_rows();
