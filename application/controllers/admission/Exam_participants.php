...
class Exam_participants extends Admin_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model(['m_exam_participants', 'm_academic_years', 'm_exam_rooms']);
		$this->pk = M_exam_participants::$pk;
		$this->table = M_exam_participants::$table;
	}

	public function index() {
		$this->vars['title'] = 'Ruang Peserta Ujian Tes Tulis';
		$this->vars['admission'] = $this->vars['exam_participants'] = TRUE;
		$this->vars['academic_year_dropdown'] = json_encode($this->m_academic_years->dropdown(), JSON_HEX_APOS | JSON_HEX_QUOT);
		$this->vars['exam_room_dropdown'] = json_encode($this->m_exam_rooms->dropdown(), JSON_HEX_APOS | JSON_HEX_QUOT);
		$this->vars['content'] = 'admission/exam_participants';
	...
	private function dataset() {
		return [
			...
			'exam_room_id' => _toInteger($this->input->post('exam_room_id', true)),
			'registrant_id' => _toInteger($this->input->post('registrant_id', true)),
			'seat_number' => $this->input->post('seat_number', true)
		];
	}
	...
	private function validation() {
		...
		$val->set_rules('exam_schedule_id', 'Jadwal Ujian', 'trim|is_natural_no_zero|required');
		$val->set_rules('exam_room_id', 'Ruang Ujian', 'trim|is_natural_no_zero|required');
		$val->set_rules('registrant_id', 'Peserta', 'trim|is_natural_no_zero|required');
