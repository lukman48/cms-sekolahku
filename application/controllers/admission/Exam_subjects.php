...
class Exam_subjects extends Admin_Controller {

	public function __construct() {
		parent::__construct();
		$this->load->model(['m_exam_subjects', 'm_academic_years']);
		$this->pk = M_exam_subjects::$pk;
		$this->table = M_exam_subjects::$table;
	}

	public function index() {
		$this->vars['title'] = 'Mata Pelajaran Ujian Tes Tulis';
		$this->vars['admission'] = $this->vars['exam_subjects'] = TRUE;
		$this->vars['academic_year_dropdown'] = json_encode($this->m_academic_years->dropdown(), JSON_HEX_APOS | JSON_HEX_QUOT);
		$this->vars['content'] = 'admission/exam_subjects';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_exam_subjects->get_where($keyword, 'rows', $limit, $offset);
			$total_rows = $this->m_exam_subjects->get_where($keyword);
	...
	private function dataset() {
		return [
			...
			'subject_order' => _toInteger($this->input->post('subject_order', true))
		];
	}
	...
	private function validation() {
		...
		$val->set_rules('subject_name', 'Mata Pelajaran', 'trim|required');
		$val->set_rules('subject_order', 'Urutan', 'trim|is_natural_no_zero|required');
		$val->set_message('required', '{field} harus diisi');
		$val->set_message('is_natural_no_zero', '{field} harus diisi dengan angka');
